...
class BMP3XX(object):
    def __init__(self):
        '''!
          @brief Base class initialization
          @note This only initializes basic parameters, communication setup happens in derived classes
        '''
        # Sea level pressure in Pa.
        self.sea_level_pressure = STANDARD_SEA_LEVEL_PRESSURE_PA
        # Initialize calibration data as None - will be populated later
        self._data_calib = None

    def begin(self):
        '''!
        @brief Initialize sensor with verification
        @return Return initialization status
        '''
        try:
            # First verify I2C communication
            chip_id = self._read_reg(BMP3XX_CHIP_ID, 1)
            logger.info(f"Detected chip ID: 0x{chip_id[0]:02x}")
            
            if chip_id[0] not in (BMP388_CHIP_ID, BMP390L_CHIP_ID):
                logger.error(f"Unexpected chip ID: 0x{chip_id[0]:02x}")
                return False
...
class BMP3XX_I2C(BMP3XX):
    ...
    def __init__(self, i2c_addr=0x77, bus=3):
        '''!
